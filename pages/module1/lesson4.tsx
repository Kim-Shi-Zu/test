...
import CodeFormatter from '@/components/projects/code-formatter';
...
export default function Lesson4({ source, frontMatter }: { source: any, frontMatter: any }) {
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Project Demo: {frontMatter.project.name}</CardTitle>
        </CardHeader>
        <CardContent>
          <CodeFormatter />
        </CardContent>
...
export async function getStaticProps() {
  const lessonPath = path.join(process.cwd(), 'content/module1/lesson_1_4.md');
  ...
  if (match) {
    ...
    frontMatterString.split('\n').forEach(line => {
      ...
      if (key && valueParts.length) {
        ...
        (frontMatter as Record<string, string>)[key.trim()] = value.replace(/^"(.*)"$/, '$1');
      }
    });

    // Handle nested objects like project
    if ((frontMatter as any).project) {
      const projectLines = frontMatterString.split('\n').filter(line => line.trim().startsWith('  '));
      (frontMatter as any).project = {};
      projectLines.forEach(line => {
        const [key, ...valueParts] = line.split(':');
        if (key && valueParts.length) {
          const nestedKey = key.trim();
          const value = valueParts.join(':').trim();
          (frontMatter as any).project[nestedKey] = value.replace(/^"(.*)"$/, '$1');
        }
      });
    }
